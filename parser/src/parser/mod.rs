...
impl<'src, 'a, A: AstDefs, F: Copy> Parser<'src, 'a, A, F> {
    ...
    pub fn parse_expr(&mut self, allow_extra: bool) -> A::Box<A::AstTrait<'src>> where (): AllImplTrait<A::AstTrait<'src>> {
        todo!()
    }
...
pub fn parse_expr<'src, A: AstDefs, F: Copy, E: ErrorReporter<SourcedError<F, ParseASTError>>>(input: &[Token<'src, SourceSpan>], file: F, mut errs: E, _defs: A) -> A::Box<A::AstTrait<'src>> {
    let mut parser = Parser::<'src, '_, A, F>::new(input, file, &mut errs);
    parser.parse_expr(false)
}
pub fn parse_tl<'src, A: AstDefs, F: Copy, E: ErrorReporter<SourcedError<F, ParseASTError>>>(input: &[Token<'src, SourceSpan>], file: F, mut errs: E, _defs: A) -> asts::FrolicAST<A::Box<A::AstTrait<'src>>, F> {
    let mut parser = Parser::<'src, '_, A, F>::new(input, file, &mut errs);
