...
fn ucb_get_blk<'a, 'src: 'a, S>(term: &'a mut Terminator<'src, S>) -> &'a mut BlockId<'src, S> {
    if let Terminator::UncondBr(br) = term { br } else { unreachable!("ICE: modified return") }
}
...
impl<'src, F: Copy, A: ToHir<'src, F>> ToHir<'src, F> for asts::IfElseAST<A> {
    fn local<'l, 'g: 'l>(
        ...
        loc.block_term().set(Terminator::UncondBr(BlockId::invalid()));
        let (if_true_val, erred) = self.if_true.local(glb, loc);

        debug_assert_eq!(loc.insert_func, f, "ICE: modified insert function in the middle of if/else");

        let if_true = loc.push_swap_blk(&glb.module, Block::new("if_false"));

        loc.block_term().set(Terminator::UncondBr(BlockId::invalid()));
        let (if_false_val, erred) = if erred { (const_err(), true) } else { self.cond.local(glb, loc) };
        ...
        func[cond_blk].term.set(Terminator::CondBr { cond, if_true, if_false });
        loc.lazy_block_id(func[if_true].term.map_ref(ucb_get_blk));
        loc.lazy_block_id(func[if_false].term.map_ref(|term| if let Terminator::UncondBr(br) = term { br } else { unreachable!("ICE: modified return") }));

        let ret = if erred { const_err() } else {
